...
export const CategoryNavPills: React.FC<CategoryNavPillsProps> = ({
  ...
  if (variant === 'cards') {
    return (
      <div className={`grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2 sm:gap-2.5 ${className}`}>
        {CATEGORIES_CONFIG.map((cat) => {
          ...
          return (
            <button
              ...
              id={`cat-card-${cat.id}`}
              className={`group flex flex-col items-center justify-center p-2.5 sm:p-3 rounded-2xl border text-center transition-all duration-200 cursor-pointer ${
                isActive
                  ? 'bg-amber-500 text-slate-950 border-amber-500 shadow-md ring-2 ring-amber-400/50 scale-[1.02]'
                  : 'bg-white hover:bg-slate-50 text-slate-800 border-slate-200 hover:border-amber-400/60 shadow-xs'
              }`}
            >
              <div
                className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center mb-1.5 transition-transform group-hover:scale-110 shrink-0 ${
                  isActive
                    ? 'bg-slate-950 text-amber-400'
                    : 'bg-amber-50 text-amber-700 border border-amber-200/60'
                }`}
              >
                {getIcon(cat.id, 'w-4 h-4 sm:w-5 sm:h-5 shrink-0')}
              </div>

              <span className="font-bold text-xs sm:text-sm tracking-tight truncate w-full px-1">
                {cat.title}
              </span>

              {cat.badge && (
                <span
                  className={`mt-1 text-[9px] px-1.5 py-0.5 rounded-full font-bold leading-none truncate max-w-full ${
                    isActive
  ...
  return (
    <div
      className={`flex items-center gap-1.5 sm:gap-2 overflow-x-auto no-scrollbar py-1 px-0.5 scroll-smooth ${className}`}
      dir="rtl"
    >
      {CATEGORIES_CONFIG.map((cat) => {
        const isActive = activeCategory === cat.id;
        ...
        return (
          <button
            ...
            onClick={() => onSelectCategory(cat.id)}
            id={`cat-pill-${cat.id}`}
            className={`shrink-0 flex items-center gap-1.5 px-3 py-1.5 sm:px-3.5 sm:py-2 rounded-xl text-xs sm:text-sm font-bold transition-all duration-200 cursor-pointer ${
              isActive
          ...
          >
            ...
            >
              {getIcon(cat.id, 'w-3.5 h-3.5 sm:w-4 sm:h-4 shrink-0')}
            </span>
