...
export const MobileBottomNav: React.FC<MobileBottomNavProps> = ({
  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-lg border-t border-slate-200 shadow-[0_-4px_25px_rgba(0,0,0,0.08)] pb-safe">
      <div className="max-w-md mx-auto grid grid-cols-5 items-center h-15 px-1.5 text-center">
        ...
        <div className="flex items-center justify-center">
          <button
            onClick={onOpenCart}
            id="mobile-nav-cart"
            className="relative -top-3 w-12 h-12 rounded-full bg-gradient-to-tr from-amber-500 to-amber-400 text-slate-950 flex flex-col items-center justify-center shadow-lg shadow-amber-500/30 border-2 border-white ring-2 ring-amber-400/50 hover:scale-105 active:scale-95 transition-transform"
          >
            <ShoppingBag className="w-5 h-5 shrink-0" />
            <span className="text-[8px] font-black leading-none mt-0.5">السلة</span>
            {cartCount > 0 && (
              <span className="absolute -top-1 -right-1 bg-slate-950 text-amber-300 text-[10px] font-black w-4.5 h-4.5 rounded-full flex items-center justify-center border-2 border-white">
                {cartCount}
              </span>
            )}
          </button>
        ...
        >
          <Crown className="w-5 h-5 mb-0.5 shrink-0 text-amber-600" />
        ...
        >
          <MessageCircle className="w-5 h-5 mb-0.5 shrink-0 text-emerald-600" />
